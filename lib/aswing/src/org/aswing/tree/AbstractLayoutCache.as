...
public class AbstractLayoutCache implements RowMapper{
    ...
    public function getPreferredWidth(bounds:IntRectangle):int {
		...
		if(rowCount > 0) {
		    ...
		    var paths:Array = getVisiblePathsFrom(firstPath, int.MAX_VALUE);
			var pn:int = paths.length;
		    if(paths != null && pn>0){
				var pBounds:IntRectangle = getBounds(TreePath(paths[0]), null);
				var width:int;
    ...
    public function getRowsForPaths(paths:Array):Array {
		...
		for(var counter:int = 0; counter < numPaths; counter++)
     ...
     */
    protected function countNodeDimensions(value:Object, row:int, depth:int,
